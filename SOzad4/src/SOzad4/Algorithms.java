...
private	int wszystkieBledy(){
	...
	private int calkowitaLiczbaOdwolan() {			//wsystkie odwolania
		int licznik = 0;
		for(Proces p: procesy)
	...
	// jezeli jakis krotszy proces sie skonczyl w naszej symulacji 
	//to przeciez jego ramki sie rowniez zwalaniaja, pula wolnych ramek wzrasta
	//wiec moze te ramki jakos wykorzystac (, te ktore sa jeszcze nie ukonczone)
	// po jakims czasie dla dluzszych procesow wiecej ramek, z puli
	public void parallelEqualAllocation(){
		...
		private void przydzielDostepneRamki(){ //przydzielaj po kolei po indeksie
	...
	private void balansuj(Proces p) {
		...
		private void kradnijRamke(Proces p){
			for(Proces proc: procesy){ //przejdz procesy
				if(proc!=p&&!proc.skonczony){ //jezeli to nie TEN proces i proces nie jest skonczony
					if(p.priorytet>proc.priorytet &&proc.przydzielonaIloscRamek>1){ //jezeli priorytet jest wiekszy i mozna ukrasc (wiecj niz 1 ramka)
						if(proc.wypelnionaTablicaRamek()){ //jezeli jest pelna
							proc.tablicaRamekProcesu.remove(proc.tablicaRamekProcesu.size()-1); //to usun z ramki ostatniego
						}
						proc.przydzielonaIloscRamek--; //zmiejsz ilosc ramek 
						p.przydzielonaIloscRamek++;
						return;
					}
				}
	...
	private void balansujGlobalnie(Proces p) {
		
		if(p.procentBledow()>=0.6){ //jezeli powyzej 60% to proces potrzebuje ramki
			if(pulaWolnychRamek>0) {// jezeli taka jest
			p.przydzielonaIloscRamek++;
			pulaWolnychRamek--;
			}
			else{
	...
	public void controllingTheFrequency(){
		...
		//na poczatku przydzielamy ramki rowno, pula wolnych ramek =0
		//w miare potrzeb usuwamy/zwiekszamy ilosc ramek, jezeli pula na to pozwala
		//jezeli nie to probujemy ukras procesowi o nizszym priorytecie
		// jezeli jakis krotszy proces sie skonczyl w naszej symulacji 
		//to przeciez jego ramki sie rowniez zwalaniaja, pula wolnych ramek wzrasta
		//wiec moze te ramki jakis proces  wykorzysta
	
	public void controllingTheFrequencyGlobal(){
		clear();
		przydzielRamkiRowno();
		...
		int nrOdwolania=0;
				...
				while(!wszystkieSkonczone()){ // rob dopoki nie sa wszystkie skonczone 
					...
					for(;nrProcesu<iloscProcesow;nrProcesu++){ // przechodz 1 odwolanie 1 proc, 1 odwolanie 2 proc ...
						...
						if(!danyProces.skonczony){ // jezeli proces jest nieskonczony to one loop lru
							LRU.oneLoopLRU(danyProces, nrOdwolania);
							balansujGlobalnie(danyProces); //jezeli potrzeba to zwieksz/zmniejsz ramki
								if(nrOdwolania==(danyProces.iloscOdwolan-1)){ //jezeli to jego ostatnie odwolanie ,skoncz go
									danyProces.skonczony=true;		
								pulaWolnychRamek+=danyProces.przydzielonaIloscRamek;
	...
	public void modelStrefowy(){
		...
		while(!wszystkieSkonczone()){
			...
			int sumaZbiorowRoboczych=ustalSumeZbiorowRoboczych(nrOdwolania); //od razu ustalilem zbiory dla procesow
	...
	private	void przydzielOptymalnieRamki(int sumaZbiorowRoboczych){
		...
		public	void modelStrefowyGlobal(){
				clear();
				przydzielRamkiRowno();
				...
				while(!wszystkieSkonczone()){
					...
					przydzielOptymalnieRamkiGlobalnie(sumaZbiorowRoboczych);
					}
					//dostalismy informacje o zbiorach, nastepnie usunelismy go, 
					//zeby znowu mogl  sie zapelnic o delte odwolan
					//w miare mozliwosci pozamieniane (oczywiscie jezeli byly wolne ramki), badz dodalismy ramki do puli
	...
	private		void przydzielOptymalnieRamkiGlobalnie(int sumaZbiorowRoboczych){
				...
				for(Proces proces:procesy){ //dla kazdego procesu oblicz jego optymalna ilosc ramek po zbiorze roboczym
					...
					if(!proces.skonczony){ //ktory nie jest skonczony
						...
						if( optimalQuantity>proces.przydzielonaIloscRamek){ //jezeli mozna dodaj ramki
							dodajOptymalnieRamkiGlobalnie(proces,optimalQuantity);
						}
			...
			//jezeli zabraknie w puli ramek to zabierz procesom o nizszym priorytecie po 1 ramce,
			//do tego momemtu do ktorego potrzebujesz(1 petla)
			private void dodajOptymalnieRamkiGlobalnie(Proces proces, int optimalQuantity){
				while(pulaWolnychRamek>0 && proces.przydzielonaIloscRamek<optimalQuantity) {
					proces.przydzielonaIloscRamek++;
					pulaWolnychRamek--;
				}
				
				if(proces.przydzielonaIloscRamek<optimalQuantity)
					
				for(Proces p: procesy){
					if(proces.przydzielonaIloscRamek==optimalQuantity)
						return;
					if(p!=proces &&!p.skonczony){
						if(proces.priorytet>p.priorytet &&p.przydzielonaIloscRamek>1){
							if(p.wypelnionaTablicaRamek()){ //jezeli jest pelna
								p.tablicaRamekProcesu.remove(p.tablicaRamekProcesu.size()-1); //to usun z ramki ostatniego
							}
							p.przydzielonaIloscRamek--; //zmiejsz ilosc ramek 
							proces.przydzielonaIloscRamek++;
